...
1. Prefer incremental transforms; persist a watermark before state-changing operations.
2. Use idempotent writes (MERGE/UPSERT) and keep source timestamps for traceability.
3. Emit structured logs and metrics and collect them centrally for SLAs and alerting.

## Patterns (with examples)

### Incremental (watermark)

When source supports watermarking or change capture.

Flow: GetWatermark → CopyNew → Transform → UpdateWatermark

Example: `DP_INGST_Orders` uses lastProcessedTimestamp to copy deltas.

### Full refresh (with backup)

Use for small tables or re-sync scenarios.

Flow: BackupTarget → Truncate → LoadFull → Validate
...
## Notebooks: recommended patterns
...
Key metrics: success rate, latency, rows/second, queue depth, cost per run
...
- Enforce private endpoints for sensitive sources
