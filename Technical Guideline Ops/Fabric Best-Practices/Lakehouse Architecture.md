...
|---|---|
| Bronze | Ingest and persist raw source data with minimal transformation and audit metadata |
| Silver | Standardize, clean, and enforce schema/quality checks |
...
graph LR
  Source[(Source Systems)] --> Bronze["Bronze<br/>raw"]
  Bronze --> Silver["Silver<br/>standardized"]
...
| Implement regular file compaction jobs (cfr. vacuum & optimize) | Let small files accumulate                   |
...
1. Start by implementing Bronze→Silver flow for a single domain. Automate schema checks and deduplication at the Silver stage.
...
|-------|---------|-----------------|--------------|
| Bronze | Land raw files | Raw tables, parquet/delta | Retention, audit cols |
| Silver | Standardize & validate | Cleansed entities | Data quality checks |
...
- Use parquet for archival where Delta isn't required.

## OneLake & Shortcuts (practical notes)

- OneLake is the logical lake that surfaces ADLS Gen2 with Fabric-specific capabilities (shortcuts, cache). Use OneLake Shortcuts to connect existing data lakes without duplicating data. There are two useful shortcut types:
  - File Shortcuts: point to files (CSV/JSON) and are best used in Bronze/landing layers. Note: File Shortcuts are not accessible via SQL endpoints.
...
- Prefer multiple lakehouses when you need isolation for security, sensitivity labels, or separate compute boundaries. Separate lakehouses make it easier to apply sensitivity labels (currently per-lakehouse) and manage granular access controls.
...
Track ingestion latency, fresheness, error rates and storage growth. Define SLA targets for critical datasets.
...
## Related pages
- [[Technical Guideline Ops/Architectural Principles/Medallion - Bronze Silver Gold|Medallion Architecture]] - Understanding Bronze/Silver/Gold vs our layering approach
- [[Technical Guideline Ops/Architectural Principles/Data Layers and Modeling|Data Layers and Modeling]] - Plainsight's preferred explicit layering
