...
!!! tip "Rule of thumb"
    Azure-only product → **Bicep**. The moment the project also needs to manage Databricks workspace objects, Fabric items, or Entra groups as code → **Terraform**, so one tool owns the whole deployment.
